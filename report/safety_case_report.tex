...
\noindent\textbf{Convergence.} Loss curves were retained for the pedestrian and
traffic-light detectors (\cref{tab:losses}); the vehicle detector's curves were lost
before they were recorded. Training loss fell monotonically for both, but only the
traffic-light detector also converged on validation ($0.113 \rightarrow 0.077$). The
pedestrian detector diverged on validation ($0.569 \rightarrow 0.714$) while its training
loss kept falling ($0.513 \rightarrow 0.269$), i.e.\ it overfits after about three epochs
--- consistent with the recall failure reported in V-1.
...
used for the confidence-gate study in V-3 (\cref{tab:tempcond}) and for all Grad-CAM
overlays. It is labelled explicitly wherever it appears, because mixing it with the
baseline detectors would break the traceability of the numbers.
...
Grad-CAM (activations of \texttt{layer4} weighted by their gradients w.r.t.\ the output)
was applied to a ResNet-18 on the same four condition frames used throughout the
exercises. It was chosen because it needs no architectural change, is faithful to the
last convolutional stage where spatial evidence is still present, and yields maps
directly comparable across conditions. The overlays in \cref{fig:expl-pedestrian} use a
contrast-stretched jet map with CAM-dependent opacity so localisation is visible.

\begin{figure}[ht]
\centering
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_day}
  \caption{In-ODD}\label{fig:expl-ped-day}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_fog}
  \caption{Fog}\label{fig:expl-ped-fog}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_night}
  \caption{Night}\label{fig:expl-ped-night}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_town}
  \caption{New town}\label{fig:expl-ped-town}
\end{subfigure}
\caption{Grad-CAM overlays for the four condition frames. Panel (a) is inside the ODD;
(b)--(d) are outside it. Red/yellow is high attribution, green is intermediate, blue is
low. The jet map is contrast-stretched and drawn with CAM-dependent opacity so a
near-constant map cannot collapse to a uniform blue wash.}
\label{fig:expl-pedestrian}
\end{figure}

\noindent\textbf{Interpretation.} The overlays support LS-1 and LS-6 directly. In-ODD
(\cref{fig:expl-ped-day}) the hot region sits on the near-field road and the ego hood,
not on any road user; the correctly classified frames of \cref{fig:expl-extra} instead
concentrate on a building facade in the upper-left corner. In both cases the decision is
driven by background or chassis structure specific to the training setup, the textbook
symptom of a spurious correlation and the reason performance transfers so poorly to
\texttt{test-town-01}. On misclassified frames the map collapses into one large blob
centred on a \emph{vehicle} in the middle of the road (\cref{fig:expl-extra}), so the
pedestrian decision is being driven by vehicle-shaped structure. Under fog, night and the
unseen town (\cref{fig:expl-ped-fog,fig:expl-ped-night,fig:expl-ped-town}) attribution
stays on the hood and the immediate pavement --- never on a distant pedestrian or a
novel-town landmark --- yet the model still returns a confident label. Near-field
attribution combined with rising confidence (V-4) is precisely the mechanism of silent
failure.
...
\textbf{FP at $\tau^{*}$} & \textbf{TN at $\tau^{*}$} & \textbf{Recall at $\tau^{*}$} \\
\midrule
Uncalibrated ($T=1.0$)       & 70\,600 & 3\,265 & 2\,765 & 129 & 0.993 \\
Temperature-scaled ($T=2.9$) & 70\,600 & \textbf{2\,894} & 2\,894 & 0 & 1.000 \\
\bottomrule
\end{tabular}
\end{center}
...
\emph{threshold}, not from calibrating ($3\,265 \rightarrow 2\,894$ is $11\,\%$, whereas
$\tau=0.5$ costs $70\,600$ either way). Both operating points are degenerate. At
$\tau=0.5$ the detector predicts ``no pedestrian'' on all 3\,600 frames (recall $0.000$),
and calibration cannot rescue it because temperature scaling is monotone in the logit and
never moves a decision taken at a fixed threshold. At $\tau^{*}$ it predicts
``pedestrian'' on all 3\,600 frames (zero true negatives, accuracy $0.196$): perfect
recall bought with unconditional braking. \textbf{Neither point is deployable}, which is
the quantitative core of the deployment recommendation.

\paragraph{Effect of $T$ on the confidence gate $\theta=0.6$.}
On the Exercise-5 pedestrian detector, accuracy is identical at $T\in\{0.5,1.0,2.0\}$
($0.7967$ in-ODD) because $T>0$ preserves the sign of the logit. What $T$ changes is how
often the constraint ``if
...
that restores recall to $1.000$ does so by braking on \emph{every} frame. V-4 is only
...
Grad-CAM overlays (V-1, \cref{fig:expl-pedestrian}, \cref{fig:expl-extra})
  & \texttt{explainability/overlay\_gradcam.py} (condition frames),
...
% the uncaptioned longtables above silently step the table counter; restart the
% numbering here so the three captioned tables read 1-3
\setcounter{table}{0}
...
were not retained in the notebook output and are therefore omitted.}
\label{tab:losses}
\end{center}
...
\begin{subfigure}[t]{0.32\linewidth}
  \centering\includegraphics[width=\linewidth]{hist_test_T05}
  \caption{in-ODD, $T=0.5$}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\linewidth}
  \centering\includegraphics[width=\linewidth]{hist_test_T10}
  \caption{in-ODD, $T=1.0$}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\linewidth}
  \centering\includegraphics[width=\linewidth]{hist_test_T20}
  \caption{in-ODD, $T=2.0$}
\end{subfigure}

\medskip
\begin{subfigure}[t]{0.32\linewidth}
  \centering\includegraphics[width=\linewidth]{hist_fog_T10}
  \caption{fog, $T=1.0$}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\linewidth}
  \centering\includegraphics[width=\linewidth]{hist_night_T10}
  \caption{night, $T=1.0$}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.32\linewidth}
  \centering\includegraphics[width=\linewidth]{hist_town_T10}
...
$0$ and $1$ and therefore fires the $\theta=0.6$ confidence gate least often. At $T=1.0$
the number of frames \emph{above} $0.6$ is 420 in-ODD but 583 at night and 760 in the
unseen town (\cref{tab:tempcond}), i.e.\ the detector is not less confident out of domain
--- only under fog (224) does it become more cautious.}
\label{fig:hist}
\end{figure}
...
\centering
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_correct_1}
  \caption{correct}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_correct_2}
  \caption{correct}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_error_1}
  \caption{misclassified}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.235\linewidth}
  \centering\includegraphics[width=\linewidth]{gradcam_error_2}
  \caption{misclassified}
\end{subfigure}
